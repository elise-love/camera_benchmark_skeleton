...
# @ray.remote
def ilniqe(img, mu_pris_param, cov_pris_param, gaussian_window, principleVectors, meanOfSampleData, resize=True, block_size_h=84, block_size_w=84, verbose=False):
    ...
    for scale in (1, 2):  # perform on two scales (1, 2)
        if verbose:
            print(f'Extracting features at scale {scale}/2...', flush=True)
...
def calculate_ilniqe(img, crop_border, input_order='HWC', num_cpus=3, resize=True, version='python', verbose=False, **kwargs):
    """Calculate IL-NIQE (Integrated Local Natural Image Quality Evaluator) metric.
    ...
    ilniqe_result = ilniqe(img, mu_pris_param, cov_pris_param, gaussian_window, principleVectors, meanOfSampleData, resize, verbose=verbose)
...
if __name__ == '__main__':
    ...
    root_dir = os.path.dirname(os.path.abspath(__file__))
    img_path = os.path.join(root_dir, 'pepper_exa', 'pepper_4.png')
    print(f'Loading image: {img_path}', flush=True)
    img = read_image_bgr(img_path)
    if img is None:
        print(f'Error: Unable to load image at {img_path}')
        exit()
    print('Calculating IL-NIQE score. This can take about a minute...', flush=True)
    with warnings.catch_warnings():
        warnings.simplefilter('ignore', category=RuntimeWarning)
        time_start = time.time()

        niqe_result = calculate_ilniqe(img, 0, input_order='HWC', resize=True, version='python', verbose=True)
...
r"""
cd "C:\Users\user\source\repos\New folder"

& "C:\Users\user\.cache\codex-runtimes\codex-primary-runtime\dependencies\python\python.exe" `
  "C:\Users\user\source\repos\New folder\score_ilniqe_folder.py" `
  "C:\Users\user\source\repos\New folder\il-niqe\IL-NIQE\data" `
  --workers 4
