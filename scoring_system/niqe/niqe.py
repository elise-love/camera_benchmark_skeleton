...
def find_pngs(path):
    if path.is_file():
        if path.suffix.lower() != '.png':
            raise ValueError("Only PNG files are supported by this runner.")
        return [path]
    if path.is_dir():
        return sorted(file for file in path.iterdir() if file.is_file() and file.suffix.lower() == '.png')
    raise FileNotFoundError("Input path does not exist: %s" % path)


def score_pngs(input_path):
    rows = []
    for image_path in find_pngs(input_path):
        try:
            score = float(niqe(image_to_gray_array(image_path)))
            rows.append({
            ...
            print("%s: %.3f" % (image_path.name, score))
            ...
            print("%s: error: %s" % (image_path.name, exc))
    return rows
...
def write_results(rows, csv_path, json_path):
    ...
    with csv_path.open("w", newline="", encoding="utf-8") as handle:
        writer = csv.DictWriter(handle, fieldnames=["image", "niqe", "status", "error"])
        writer.writeheader()
...
def parse_args():
    ...
    default_input = module_path / "data_image"
    default_csv = default_input / "niqe_scores.csv"
    default_json = default_input / "niqe_scores.json"

    parser = argparse.ArgumentParser(description="Run NIQE on PNG images.")
    parser.add_argument(
        "input",
        nargs="?",
        type=Path,
        default=default_input,
        help="PNG file or folder of PNG files. Defaults to ./data_image.",
    )
...
def main():
    ...
    rows = score_pngs(input_path)

    if not rows:
        print("No PNG files found in %s" % input_path)
        return

    write_results(rows, args.csv.resolve(), args.json.resolve())
    print("Wrote CSV: %s" % args.csv.resolve())
    print("Wrote JSON: %s" % args.json.resolve())
