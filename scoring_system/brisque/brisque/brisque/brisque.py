...
try:
    ...
    # Support direct execution of this module from Visual Studio.
    from models import MODEL_PATH
...
class BRISQUE:
    ...
    def scale_features(self, features):
        ...
        features_flat = np.array([scalar(f) for f in features], dtype=np.float64)
        min_flat = np.array([scalar(m) for m in min_], dtype=np.float64)
        max_flat = np.array([scalar(m) for m in max_], dtype=np.float64)
...
if __name__ == "__main__":
    ...
    batch_script = Path(__file__).resolve().parents[2] / "score_brisque_pngs.py"
    print(f"Running PNG batch scorer: {batch_script}\n", flush=True)
